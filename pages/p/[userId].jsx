...
const ProfilePage = () => {
  ...
  return !userData ? (
    ...
    <div className="flex flex-col bg-gray-300 w-full min-h-screen mb-20">
      <div>
        <div className="flex flex-col w-full bg-white">
          <div className="flex flex-row items-center w-full mt-2 px-2 py-1">
            <div className="flex items-center">
              <IoIosArrowBack
                ...
                onClick={() => router.back()}
              />
              <span className="text-[#0056FF] font-bold text-md ml-2">
          ...
          <div className="flex flex-row justify-evenly items-center px-8 py-4 w-full">
            ...
            <div className="flex flex-col justify-between gap-2 items-center w-full">
              <span className="text-[#0056FF] font-bold text-xl">
                {userData?.user?.fullname}
              </span>
                ...
                {userData?.user?.userId === session?.user?.id ? (
                  <>
                    <div className="flex flex-row px-4 py-1 bg-[#0056FF] text-white text-sm rounded-full items-center gap-1 ">
                      <span>
                        Point: <strong>{userData?.points?.point}</strong>
                      </span>
                    </div>
                    <div className="flex flex-row px-4 py-1 bg-[#F2871F] text-white text-sm rounded-full items-center gap-1">
                      <span>
                        Coins: <strong>{userData?.coins?.coins}</strong>
                      </span>
                    </div>
                  </>
                ) : (
                  <>
                    <Follower targetId={userData?.user?.userId} />
                    <div
        ...
        <div className="flex flex-col justify-center items-center mt-1 bg-white p-2 w-full">
          ...
          <div className="flex flex-row justify-between items-center gap-2 w-full">
            {badges.data.map((badge, index) => (
              <Tooltip key={index} title={badge?.name} placement="top" arrow>
                <div key={index} className="relative w-[50px] h-auto">
                  <Image
                    src={badge?.image?.url}
                    alt="badge"
                    width={80}
                    height={80}
                    className="grayscale opacity-20 object-contain"
                    priority
                  />
                </div>
              </Tooltip>
        ...
        <div className="flex flex-col mt-1 bg-white p-2 w-full">
          <ul className="flex flex-row justify-between items-center text-sm text-[#0056FF]">
            <div className="flex flex-row items-center w-2/6">
              <span className="font-bold text-lg">รายละเอียด</span>
            </div>
            <div className="flex flex-row justify-evenly items-center text-sm text-gray-500 w-full">
              <li
                className={`cursor-pointer  ${
                  tabs === "posts"
                    ? "border-b-2 border-[#F2871F] text-[#0056FF] font-bold"
                    : ""
                }`}
          ...
          <div className="flex flex-col mt-1 bg-white p-2 text-xs w-full">
            <span className="text-[#0056FF] font-bold">
              TeamGroup: {userData?.emp?.teamGrop}
            </span>
            <span className="text-[#0056FF] font-bold ">
              {userData?.emp?.position
                ? "Position: " + userData?.emp?.position
                : ""}
            </span>
            <span className="text-[#0056FF] font-bold ">
              {userData?.emp?.group ? "Group: " + userData?.emp?.group : ""}
            </span>
            <span className="text-[#0056FF] font-bold ">
              {userData?.emp?.department
                ? "Department: " + userData?.emp?.department
                : ""}
            </span>
            <span className="text-[#0056FF] font-bold ">
              {userData?.emp?.email ? "e-mail: " + userData?.emp?.email : ""}
            </span>
          </div>
        </div>
        ...
        {tabs === "posts" && (
          <div className="flex flex-col w-full">
            <div className="flex flex-col mt-1 bg-white p-2 w-full">
              <>
                <span className="text-[#0056FF] font-bold text-lg">โพสต์</span>
      ...
      {tabs === "picture" && (
        <ImageTab userId={userId} ImageData={userData?.images} />
      )}
      {tabs === "video" && (
