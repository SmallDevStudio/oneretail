...
const SurveyPosition = () => {
    ...
    const fetchSurveyData = async () => {
        try {
            const response = await axios.get(`/api/survey/board/pb/position`, {
            ...
            {selectedPosition && (
                ...
                >
                    <h3 
                        ...
                        onClick={() => router.push(`/survey/PB/${selectedPosition}?startDate=${startDate}&endDate=${endDate}`)}
                    >
                        รายละเอียดสำหรับเขต: {selectedPosition}
                    </h3>
                    <span 
                    ...
                    {surveyDetails
                            ...
                            .filter(detail => {
                                const position = positionData.find(position => position.position === selectedPosition);
                                const total = position?.counts[detail.value] || 0;
                                return total > 0; // Only include items with total > 0
                            })
                            .map(detail => {
                                const position = positionData.find(position => position.position === selectedPosition);
                                const total = position?.counts[detail.value] || 0;
                                const verbatim = position?.memoCount[detail.value] || 0;
                                    ...
                                    <li key={detail.value} style={{ color: detail.color }}>
                                        <div className="flex flex-row gap-4">
                                            ...
                                            <span
                                                ...
                                                onClick={() => 
                                                    router.push(
                                                        `/survey/verbatim?teamGrop=${teamGrop}&position=${selectedPosition}&startDate=${startDate}&endDate=${endDate}&value=${detail.value}`
                                                    )
                                                }
                                            >
...
export default SurveyPosition;

SurveyPosition.getLayout = (page) => <AppLayout>{page}</AppLayout>;
SurveyPosition.auth = true