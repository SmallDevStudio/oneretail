...
const SurveyGroup = () => {
    ...
    const { chief_th, group, startDate, endDate } = router.query;
    const teamGrop = 'AL';
    ...
    const fetchSurveyData = async () => {
        try {
            const response = await axios.get(`/api/survey/board/al/department`, {
    ...
    useEffect(() => {
        if (teamGrop && chief_th &&group) {
            fetchSurveyData();
        }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [startDate, endDate, teamGrop, chief_th, group]);
            ...
            {selectedDepartment && (
                ...
                >
                    <h3 
                        ...
                        onClick={() => router.push(`/survey/verbatim?teamGrop=${teamGrop}&chief_th=${chief_th}&group=${group}&startDate=${startDate}&endDate=${endDate}`)}
                    >
                                    ...
                                    <li key={detail.value} style={{ color: detail.color }}>
                                        <div className="flex flex-row gap-4">
                                            ...
                                            <span
                                                ...
                                                onClick={() => 
                                                    router.push(
                                                        `/survey/verbatim?teamGrop=${teamGrop}&chief_th=${chief_th}&group=${group}&department=${selectedDepartment}&startDate=${startDate}&endDate=${endDate}&value=${detail.value}`
                                                    )
                                                }
                                            >
