...
const MainPage = () => {
  ...
  const {
    ...
    error: levelError,
    mutate,
  } = useSWR(session ? `/api/level/user?userId=${userId}` : null, fetcher);
  const {
    data: coins,
    error: coinsError,
    mutate: mutateCoins,
  } = useSWR("/api/coins/user?userId=" + session?.user?.id, fetcher);
  const {
    data: ads,
    error: adsError,
    mutate: mutateAds,
  } = useSWR("/api/ads/main", fetcher);
      ...
      <RecheckUser>
        <main className="flex flex-col bg-gray-10 justify-between items-center text-center min-h-screen">
          ...
          <div className="flex items-center justify-center w-full px-6 pb-2 max-w-[100vw]">
            <MainIconMenu setLinkModal={setLinkModal} />
          </div>
          ...
          <div className="relative w-full footer-content">
            ...
            <div className="text-center text-xs text-gray-300 mb-10">
              ...
              <span className="">
                {" "}
                Powered by{" "}
                <span className="text-[#0056FF]/50 font-bold">One Retail</span>
              </span>
          ...
          {openAds && ads?.data?.[currentAdIndex] && (
            <AdsModal isOpen={openAds} onClose={handleCloseAds}>
              <div className="flex-col w-full h-full flex">
                ...
                <div className="flex-col w-full h-full flex justify-center items-center">
                  <Image
                    ...
                    onClick={() =>
                      ads.data[currentAdIndex].url
                        ? router.push(ads.data[currentAdIndex].url)
                        : null
                    }
                  />
                </div>
