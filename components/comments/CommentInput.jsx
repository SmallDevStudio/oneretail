...
const CommentInput = ({
  ...
  const handleSubmitComment = async () => {
    ...
    const newPost = {
      ...
      media,
      files: files
        ? [{ url: files.url, public_id: files.public_id, type: "file" }]
        : [],
      sticker,
      link,
      selectedUsers,
  ...
  return (
    <div>
      ...
      <div>
        ...
        <textarea
          className={`w-full border-gray-300 rounded-lg outline-none px-1
        ...
        <div className="flex flex-col gap-2 mt-2 mb-2">
          ...
          {files && (
            <div
              key={files.name}
              className="relative flex-col p-2 border-2 rounded-xl"
            >
              <IoIosCloseCircle
                className="absolute top-0 right-0 text-xl cursor-pointer"
            ...
            <div className="relative flex-col p-2 border-2 rounded-xl">
              <div className="flex flex-row w-full">
                ...
                <div className="flex flex-col w-full ml-2">
                  <Link
                    href={linkPreview.hybridGraph.url}
                    target="_blank"
                    className="text-xs cursor-pointer"
                  >
                    <span className="font-bold text-xs">
                      {linkPreview.hybridGraph.title}
                    </span>
                  </Link>
                  <span className="text-xs line-clamp-2">
                    {linkPreview.hybridGraph.description}
                  </span>
                </div>
              </div>
            </div>
