export default function MainIconMenu() {
    return (
        <div className="flex justify-center items-center p-5 ">
                    <div className="grid grid-cols-3 gap-4 p-2">
                            <button 
                                className="flex-col inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-x-hidden text-sm font-bold rounded-2xl border-4 border-[#0056FF] bg-white"
                                style={{ width: 95, height: 95 }}
                                onClick={() => window.location.href = "/games"}
                            >
                                <svg className="w-10 h-10 text-[#F68B1F]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 376.66 236.28">
                                    <path fill="currentColor" d="M36.11,236.28c-8.56,0-16.35-2.39-23.01-8.08-.04-.03-.07-.06-.1-.09-.67-.58-1.25-1.13-1.8-1.68-23.75-23.74-5.04-75.29,13.05-125.16,3.59-9.9,6.98-19.25,9.9-28.02,8.97-26.92,21.8-45.73,39.23-57.5C89.19,5.06,108.57.08,134.36.06c4.11,0,29.08-.02,54.04-.03,24.94-.01,49.87-.02,53.99-.03h.09c32.3,0,55.43,8.09,72.81,25.46,11.57,11.57,20.5,27.15,27.29,47.63,2.91,8.76,6.29,18.1,9.88,27.99,18.54,51.18,37.71,104.1,11.08,126.9-.06.05-.12.1-.17.15-25.8,21.77-68.45-5.46-106.09-29.48-6.61-4.22-12.86-8.21-18.56-11.62-12.66-7.55-24.14-8.39-45.52-8.39h-2.4c-2.54,0-6.14,0-7.27,0-21.37.02-32.86.86-45.56,8.46-5.73,3.43-12.01,7.45-18.67,11.7-27.95,17.86-58.67,37.49-83.18,37.49ZM29.38,209.23c4.21,3.52,13.89,2.33,27.28-3.38,15.43-6.57,33.35-18.02,49.16-28.13,6.81-4.35,13.25-8.46,19.29-12.08,18.58-11.12,35.95-11.99,58.46-12.01,1.18,0,4.7,0,7.2,0h2.32s.07,0,.11,0c22.46,0,39.8.87,58.34,11.93,6.03,3.61,12.42,7.69,19.19,12.01,15.81,10.09,33.74,21.54,49.18,28.1,13.46,5.72,23.19,6.9,27.39,3.3.02-.02.04-.03.05-.05,4.93-4.32,5.69-16.39,2.14-33.99-3.95-19.56-12.38-42.83-20.53-65.34-3.64-10.05-7.08-19.55-10.1-28.63-5.54-16.7-12.48-29.07-21.23-37.82-12.55-12.54-29.55-18.14-55.12-18.14h-.08c-4.11,0-29.05.02-54,.03-24.96.01-49.92.02-54.03.03-35.71.03-60.97,9.42-76.52,56.09-3.02,9.08-6.47,18.59-10.12,28.65-8,22.05-16.28,44.86-20.33,64.19-5.03,23.95-1.24,32.06,1.47,34.77.17.17.34.32.5.47Z"/>
                                    <path fill="currentColor" d="M132.36,112.3h12.76c6.29,0,11.38-5.1,11.38-11.38s-5.09-11.37-11.37-11.37h-12.76v-12.76c0-6.29-5.09-11.37-11.37-11.37s-11.38,5.1-11.38,11.38v12.76s-12.76,0-12.76,0c-6.29,0-11.38,5.1-11.37,11.37s5.1,11.38,11.37,11.37h12.76v13.12c0,6.29,5.09,11.37,11.37,11.37s11.38-5.1,11.38-11.38v-13.12Z"/>
                                    <circle fill="currentColor" cx="238.44" cy="99.96" r="12.31"/>
                                    <circle fill="currentColor" cx="263.08" cy="75.33" r="12.31"/>
                                    <circle fill="currentColor" cx="288.22" cy="100.47" r="12.31"/>
                                    <circle fill="currentColor" cx="262.82" cy="123.25" r="12.31"/>
                                </svg>
                                <span className="inline-block text-sm font-bold text-[#0056FF]">
                                    เกมส์
                                </span>
                            </button>

                        <button
                            className="flex-col inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-x-hidden text-sm font-bold text-gray-900 rounded-2xl border-4 border-[#0056FF] bg-white"
                            style={{ width: 95, height: 95 }}
                            onClick={() => window.location.href = "/training"}
                        >
                            <svg className="w-8 h-8 mb-1.5 text-[#F68B1F]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 287.01 320.21">
                                <path fill="currentColor" d="M235.52,320.21H51.49c-28.39,0-51.49-23.1-51.49-51.49V84.69c0-28.39,23.1-51.49,51.49-51.49h184.03c28.39,0,51.49,23.1,51.49,51.49v184.03c0,28.39-23.1,51.49-51.49,51.49ZM51.49,58.2c-14.61,0-26.49,11.88-26.49,26.49v184.03c0,14.61,11.88,26.49,26.49,26.49h184.03c14.61,0,26.49-11.88,26.49-26.49V84.69c0-14.61-11.88-26.49-26.49-26.49H51.49Z"/>
                                <path fill="currentColor" d="M270.26,136.56H12.5c-6.9,0-12.5-5.6-12.5-12.5s5.6-12.5,12.5-12.5h257.76c6.9,0,12.5,5.6,12.5,12.5s-5.6,12.5-12.5,12.5Z"/>
                                <circle fill="currentColor" cx="80.95" cy="180.17" r="17.94"/>
                                <circle fill="currentColor" cx="143.51" cy="180.17" r="17.94"/>
                                <circle fill="currentColor" cx="206.06" cy="180.17" r="17.94"/>
                                <circle fill="currentColor" cx="80.95" cy="233.02" r="17.94"/>
                                <circle fill="currentColor" cx="143.51" cy="233.02" r="17.94"/>
                                <circle fill="currentColor" cx="206.06" cy="233.02" r="17.94"/>
                                <path fill="currentColor" d="M84.43,58.2c-6.9,0-12.5-5.6-12.5-12.5V12.5c0-6.9,5.6-12.5,12.5-12.5s12.5,5.6,12.5,12.5v33.2c0,6.9-5.6,12.5-12.5,12.5Z"/>
                                <path fill="currentColor" d="M202.58,58.2c-6.9,0-12.5-5.6-12.5-12.5V12.5c0-6.9,5.6-12.5,12.5-12.5s12.5,5.6,12.5,12.5v33.2c0,6.9-5.6,12.5-12.5,12.5Z"/>
                            </svg>
                            <span className="inline-block text-sm font-bold text-[#0056FF]">ตารางอบรม</span>
                        </button>

                        <button
                            className="flex-col inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-x-hidden text-sm font-bold text-gray-900 rounded-2xl border-4 border-[#0056FF] bg-white"
                            style={{ width: 95, height: 95 }}
                            onClick={() => window.location.href = "/redeem"}
                        >
                            <svg className="w-8 h-8 mb-1.5 text-[#F68B1F]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 339.74 322.34">
                                <path fill="currentColor" d="M283.52,182.45H56.21c-8.93,0-13.42,0-18-.91-18.9-3.76-33.54-18.4-37.3-37.3-.91-4.58-.91-9.07-.91-18s0-13.42.91-18c3.76-18.9,18.4-33.54,37.3-37.3,4.58-.91,9.07-.91,18-.91h227.31c8.94,0,13.42,0,18.01.91,18.89,3.76,33.54,18.4,37.29,37.3.91,4.59.91,9.07.91,18s0,13.42-.91,18c-3.76,18.9-18.4,33.54-37.3,37.3-4.59.91-9.07.91-18,.91ZM56.21,95.02c-7.06,0-10.96,0-13.12.43-8.95,1.78-15.88,8.71-17.66,17.66-.43,2.17-.43,6.06-.43,13.13s0,10.96.43,13.13c1.78,8.95,8.71,15.88,17.65,17.66,2.17.43,6.06.43,13.13.43h227.31c7.07,0,10.96,0,13.13-.43,8.95-1.78,15.88-8.71,17.66-17.66.43-2.17.43-6.06.43-13.13s0-10.96-.43-13.13c-1.78-8.94-8.71-15.87-17.65-17.65-2.17-.43-6.06-.43-13.13-.43H56.21Z"/>
                                <path fill="currentColor" d="M239.81,322.34H99.93c-35.23,0-54.63,0-68.54-13.9-13.9-13.9-13.9-33.31-13.9-68.54v-69.94c0-6.9,5.6-12.5,12.5-12.5s12.5,5.6,12.5,12.5v69.94c0,30.7.27,44.54,6.58,50.86,6.32,6.32,20.16,6.58,50.86,6.58h139.88c30.7,0,44.54-.27,50.86-6.58s6.58-20.16,6.58-50.86v-69.94c0-6.9,5.6-12.5,12.5-12.5s12.5,5.6,12.5,12.5v69.94c0,35.23,0,54.63-13.9,68.54-13.9,13.9-33.31,13.9-68.54,13.9Z"/>
                                <path fill="currentColor" d="M169.87,322.34c-6.9,0-12.5-5.6-12.5-12.5V82.52c0-6.9,5.6-12.5,12.5-12.5s12.5,5.6,12.5,12.5v227.31c0,6.9-5.6,12.5-12.5,12.5Z"/>
                                <path fill="currentColor" d="M152.38,95.02h-55.46c-19.54,0-36.32-11.22-43.8-29.28-7.5-18.11-3.55-37.97,10.3-51.83C72.97,4.37,86.18-.67,99.67.07c13.46.75,26.01,7.21,34.43,17.73l34.61,43.26c5.07,6.33,6.03,14.81,2.52,22.11-3.51,7.31-10.73,11.85-18.84,11.85ZM149.19,76.68h0,0ZM96.98,25c-6,0-11.59,2.32-15.88,6.6-6.67,6.67-8.49,15.86-4.88,24.58,3.59,8.67,11.33,13.85,20.7,13.85h46.93l-29.29-36.61c-4.04-5.05-9.82-8.02-16.29-8.38-.44-.02-.87-.04-1.3-.04Z"/>
                                <path fill="currentColor" d="M242.81,95.02h-55.46c-8.11,0-15.33-4.54-18.84-11.85-3.51-7.31-2.55-15.78,2.52-22.11l34.61-43.26C214.06,7.28,226.61.82,240.07.07c13.49-.74,26.69,4.3,36.24,13.85,13.86,13.86,17.8,33.71,10.3,51.83-7.48,18.06-24.26,29.28-43.8,29.28ZM195.88,70.02h46.93c9.37,0,17.11-5.18,20.7-13.85,3.61-8.72,1.79-17.91-4.88-24.58-4.59-4.59-10.7-6.92-17.18-6.56-6.47.36-12.25,3.33-16.29,8.38l-29.29,36.61ZM180.79,68.87h0,0Z"/>
                            </svg>
                            <span className="text-xs text-bold text-nowrap text-[#0056FF]">แลกของรางวัล</span>
                        </button>

                        <button
                            className="flex-col inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-x-hidden text-sm font-bold text-gray-900 rounded-2xl border-4 border-[#0056FF] bg-white"
                            style={{ width: 95, height: 95 }}
                            onClick={() => window.location.href = "/learning"}
                        >
                            <svg className="w-8 h-8 mb-1 text-[#F68B1F]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 374.19 308.55">
                                <path fill="currentColor" d="M299.67,257.79H70.69c-20.29-.02-32.27-.28-42.68-5.59-9.71-4.94-17.46-12.7-22.41-22.43-5.59-10.97-5.59-22.86-5.59-46.55v-108.63c0-23.71,0-35.6,5.59-46.58,4.95-9.71,12.7-17.46,22.41-22.41C38.98,0,50.88,0,74.58,0h225.03c23.69,0,35.58,0,46.55,5.59,9.73,4.96,17.49,12.71,22.43,22.42,5.59,10.96,5.59,22.84,5.59,46.51v108.76c0,23.66,0,35.53-5.59,46.5-4.88,9.58-12.85,17.55-22.43,22.43-10.96,5.59-22.84,5.59-46.5,5.59ZM70.71,232.79h228.96c19.04,0,29.53,0,35.15-2.86,4.92-2.5,9-6.59,11.51-11.51,2.86-5.62,2.86-16.11,2.86-35.15v-108.76c0-19.05,0-29.54-2.86-35.16-2.53-4.97-6.51-8.94-11.51-11.49-5.63-2.87-16.13-2.87-35.2-2.87H74.58c-19.09,0-29.6,0-35.22,2.87-4.98,2.54-8.95,6.51-11.49,11.49-2.87,5.63-2.87,16.14-2.87,35.23v108.63c0,19.07,0,29.58,2.87,35.2,2.55,5,6.52,8.98,11.49,11.51,5.05,2.57,13.43,2.85,31.35,2.86Z"/>
                                <path fill="currentColor" d="M142.64,160.64v-63.49c0-3.4.91-6.74,2.63-9.67,1.73-2.93,4.2-5.35,7.18-6.99,2.97-1.65,6.34-2.47,9.74-2.38,3.4.09,6.71,1.09,9.6,2.89l50.79,31.75c2.74,1.71,5,4.09,6.57,6.92,1.57,2.83,2.39,6.01,2.39,9.24s-.82,6.41-2.39,9.24c-1.57,2.83-3.83,5.21-6.57,6.92l-50.79,31.75c-2.88,1.8-6.2,2.8-9.6,2.89-3.4.09-6.76-.73-9.74-2.38-2.97-1.65-5.45-4.06-7.18-6.99-1.73-2.93-2.63-6.27-2.63-9.67Z"/>
                                <path fill="currentColor" d="M361.11,308.55H13.08c-6.9,0-12.5-5.6-12.5-12.5s5.6-12.5,12.5-12.5h348.03c6.9,0,12.5,5.6,12.5,12.5s-5.6,12.5-12.5,12.5Z"/>
                            </svg>
                            <span className="text-sm text-bold text-nowrap text-[#0056FF]">Learn มันส์</span>
                        </button>

                        <button
                            className="flex-col inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-x-hidden text-sm font-bold text-gray-900 rounded-2xl border-4 border-[#0056FF]"
                            style={{ width: 95, height: 95 }}
                            onClick={() => window.location.href = "/leaderboard"}
                        >
                            <svg className="w-10 h-10 text-[#F68B1F]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 371.11 357.77">
                                <path fill="currentColor" d="M243.24,357.77h-115.37c-6.9,0-12.5-5.6-12.5-12.5v-161.52c0-13.25,10.78-24.04,24.04-24.04h92.29c13.25,0,24.04,10.78,24.04,24.04v161.52c0,6.9-5.6,12.5-12.5,12.5ZM140.37,332.77h90.37v-148.05h-90.37v148.05Z"/>
                                <path fill="currentColor" d="M347.07,357.77h-103.83c-6.9,0-12.5-5.6-12.5-12.5v-55.76c0-13.25,10.78-24.04,24.04-24.04h92.3c13.25,0,24.04,10.78,24.04,24.04v44.22c0,13.25-10.78,24.04-24.04,24.04ZM255.74,332.77h90.37v-42.3h-90.37v42.3Z"/>
                                <path fill="currentColor" d="M127.87,357.77H24.04c-13.25,0-24.04-10.78-24.04-24.04v-82.68c0-13.25,10.78-24.04,24.04-24.04h92.29c13.25,0,24.04,10.78,24.04,24.04v94.22c0,6.9-5.6,12.5-12.5,12.5ZM25,332.77h90.37v-80.75H25v80.75Z"/>
                                <path fill="currentColor" d="M189.49,272.62c-6.9,0-12.5-5.6-12.5-12.5v-33.13h-4.81c-6.9,0-12.5-5.6-12.5-12.5s5.6-12.5,12.5-12.5h17.31c6.9,0,12.5,5.6,12.5,12.5v45.63c0,6.9-5.6,12.5-12.5,12.5Z"/>
                                <path fill="currentColor" d="M108.1,49.02l14.11,88.17c.82,5.13,5.25,8.91,10.45,8.91h105.8c5.2,0,9.63-3.78,10.45-8.91l14.11-88.17c1.46-9.14-8.74-15.63-16.4-10.42l-28.03,19.06-25.55-25.55c-4.13-4.13-10.83-4.13-14.96,0l-25.55,25.55-28.03-19.06c-7.65-5.21-17.86,1.28-16.4,10.42ZM147.86,80.08c4.2,2.86,9.84,2.32,13.43-1.27l24.26-24.26,24.26,24.26c3.59,3.59,9.23,4.12,13.43,1.27l15-10.2-8.81,55.06h-87.76l-8.81-55.06,15,10.2Z"/>
                                <path fill="currentColor" d="M238.46,146.64h-105.8c-5.5,0-10.12-3.94-10.99-9.37l-14.11-88.17c-.71-4.45,1.21-8.72,5.03-11.14,3.81-2.42,8.5-2.35,12.22.18l27.65,18.8,25.23-25.23c2.1-2.1,4.9-3.26,7.87-3.26s5.77,1.16,7.87,3.26l25.23,25.23,27.65-18.8c3.72-2.54,8.41-2.6,12.22-.18,3.81,2.42,5.74,6.69,5.03,11.14l-14.11,88.17c-.87,5.43-5.49,9.37-10.99,9.37ZM118.57,37.29c-1.86,0-3.72.53-5.4,1.6-3.44,2.18-5.17,6.03-4.53,10.04l14.11,88.17c.78,4.89,4.95,8.45,9.91,8.45h105.8c4.96,0,9.12-3.55,9.91-8.45l14.11-88.17c.64-4.01-1.1-7.86-4.53-10.04-3.44-2.18-7.66-2.12-11.01.16l-28.4,19.31-.32-.32-25.55-25.55c-3.91-3.91-10.28-3.91-14.19,0l-25.87,25.87-28.4-19.31c-1.72-1.17-3.67-1.76-5.62-1.76ZM229.9,125.48h-88.69l-.07-.46-9.01-56.31,16.05,10.91c3.97,2.7,9.33,2.2,12.74-1.2l24.65-24.65,24.65,24.65c3.4,3.4,8.76,3.91,12.73,1.2l16.05-10.91-9.08,56.77ZM142.14,124.39h86.82l8.54-53.34-13.95,9.48c-4.41,3-10.35,2.44-14.13-1.33l-23.87-23.87-23.87,23.87c-3.77,3.77-9.71,4.33-14.13,1.33l-13.95-9.49,8.54,53.34Z"/>
                                <path fill="currentColor" d="M185.55,19.3c5.33,0,9.65-4.32,9.65-9.65s-4.32-9.65-9.65-9.65-9.65,4.32-9.65,9.65,4.32,9.65,9.65,9.65Z"/>
                                <path fill="currentColor" d="M253.12,28.96c5.33,0,9.65-4.32,9.65-9.65s-4.32-9.65-9.65-9.65-9.65,4.32-9.65,9.65,4.32,9.65,9.65,9.65Z"/>
                                <path fill="currentColor" d="M117.99,28.96c5.33,0,9.65-4.32,9.65-9.65s-4.32-9.65-9.65-9.65-9.65,4.32-9.65,9.65,4.32,9.65,9.65,9.65Z"/>
                            </svg>
                            <span className="text-sm text-bold text-nowrap text-[#0056FF]">ตารางอันดับ</span>
                        </button>

                        <button
                            className="flex-col inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-x-hidden text-sm font-bold text-gray-900 rounded-2xl border-4 border-[#0056FF] bg-white"
                            style={{ width: 95, height: 95 }}
                            onClick={() => window.location.href = "/campaign"}
                        >
                            <svg className="w-8 h-8 mb-1 text-[#F68B1F]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 387.64 401.16">
                                <path fill="currentColor" d="M295.32,61.17c-2.23,0-4.48-.6-6.52-1.84-5.88-3.61-7.73-11.3-4.12-17.19l14.51-23.67c3.61-5.89,11.31-7.73,17.19-4.12,5.88,3.61,7.73,11.3,4.12,17.19l-14.51,23.67c-2.36,3.85-6.46,5.97-10.67,5.97Z"/>
                                <path fill="currentColor" d="M375.14,204.56c-1.27,0-2.57-.2-3.84-.61l-24.2-7.81c-6.57-2.12-10.17-9.17-8.05-15.74,2.12-6.57,9.17-10.17,15.74-8.05l24.2,7.81c6.57,2.12,10.17,9.17,8.05,15.74-1.71,5.29-6.62,8.66-11.89,8.66Z"/>
                                <path fill="currentColor" d="M318.13,129.72c-4.95,0-9.63-2.96-11.59-7.82-2.58-6.4.51-13.69,6.91-16.27l32.81-13.25c6.4-2.58,13.69.51,16.27,6.91,2.58,6.4-.51,13.69-6.91,16.27l-32.81,13.25c-1.53.62-3.12.91-4.68.91Z"/>
                                <path fill="currentColor" d="M42.58,222.51l15.05,36.56-8.23,3.39c-1.88.77-3.82,1.14-5.73,1.14-5.93,0-11.55-3.52-13.94-9.34l-3.58-8.69c-3.17-7.7.5-16.5,8.2-19.67l8.22-3.39M42.58,197.51c-3.23,0-6.47.63-9.52,1.88l-8.23,3.39c-9.9,4.07-17.61,11.76-21.73,21.64-4.12,9.88-4.14,20.77-.07,30.67l3.58,8.69c6.2,15.08,20.75,24.82,37.06,24.82,5.25,0,10.38-1.02,15.25-3.02l8.23-3.39c12.77-5.26,18.86-19.87,13.6-32.63l-15.05-36.56c-2.52-6.13-7.38-11.01-13.5-13.56-3.08-1.28-6.35-1.92-9.62-1.92h0Z"/>
                                <path fill="currentColor" d="M142.97,285.49l31.17,75.72c1.27,3.08,1.06,6.51-.56,9.41-1.62,2.91-4.44,4.87-7.72,5.4-.59.09-1.17.14-1.75.14-4.33,0-8.29-2.59-9.99-6.71l-31.11-75.58,19.97-8.38M142.97,260.49c-3.29,0-6.58.65-9.67,1.95l-19.97,8.38c-12.67,5.32-18.67,19.86-13.44,32.57l31.11,75.58c5.55,13.48,18.55,22.19,33.11,22.19,1.91,0,3.84-.15,5.74-.46,10.87-1.76,20.19-8.27,25.56-17.88,5.37-9.59,6.05-20.94,1.86-31.12l-31.17-75.72c-2.53-6.16-7.42-11.05-13.58-13.59-3.05-1.26-6.3-1.89-9.54-1.89h0Z"/>
                                <path fill="currentColor" d="M210.68,274.04c1.78,16.73-7.54,33.36-23.87,40.08l-2.5,1.03-13.85-33.64,40.21-7.47M210.68,249.04c-1.52,0-3.05.14-4.57.42l-40.21,7.47c-7.4,1.38-13.79,6.02-17.39,12.63-3.6,6.61-4.02,14.5-1.16,21.46l13.85,33.64c2.52,6.13,7.38,11.01,13.51,13.56,3.08,1.28,6.34,1.92,9.61,1.92s6.48-.63,9.53-1.89l2.5-1.03c13.01-5.35,23.66-14.62,30.77-26.77,6.86-11.73,9.86-25.61,8.43-39.07-.74-6.93-4.33-13.24-9.92-17.4-4.35-3.24-9.6-4.96-14.94-4.96h0Z"/>
                                <path fill="currentColor" d="M102.66,291.29c-15.87,0-30.69-9.59-36.89-24.65l-25.29-61.44c-7.13-17.32-1.06-37.54,14.43-48.07L195.05,61.81c3.11-2.12,7.01-2.72,10.62-1.64,3.61,1.08,6.53,3.73,7.97,7.21l72.13,175.23c1.43,3.48,1.22,7.42-.58,10.73-1.8,3.31-4.99,5.63-8.7,6.32l-166.63,30.96c-2.4.45-4.81.66-7.2.66ZM196.38,91.14l-127.41,86.66c-5.76,3.92-8.02,11.44-5.37,17.88l25.29,61.45c2.65,6.44,9.54,10.2,16.4,8.92l151.5-28.15-60.41-146.76Z"/>
                                <path fill="currentColor" d="M218.04,25c5.33,0,10.39,3.16,12.54,8.39l84.76,205.93c2.84,6.9-.46,14.83-7.37,17.67-1.68.69-3.42,1.02-5.14,1.02-5.33,0-10.39-3.16-12.54-8.39L205.54,43.69c-2.84-6.9.46-14.83,7.37-17.67,1.68-.69,3.42-1.02,5.13-1.02M218.04,0h-.01c-5.04,0-9.97.98-14.64,2.9-19.65,8.09-29.06,30.66-20.97,50.31l84.76,205.93c5.97,14.5,19.97,23.87,35.66,23.87,5.04,0,9.97-.98,14.65-2.9,19.66-8.09,29.06-30.66,20.97-50.31L253.7,23.87c-5.97-14.5-19.97-23.87-35.66-23.87h0Z"/>
                            </svg>
                            <span className="text-sm text-bold text-nowrap text-[#0056FF]">แคมเปญ</span>
                        </button>

                    </div>
                </div>
    );
}