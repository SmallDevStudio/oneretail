...
export default function NewsComment({ comment, userId, onClose }) {
  const [openDialog, setOpenDialog] = useState(false);
  const [currentReply, setCurrentReply] = useState(null);
  const [liked, setLiked] = useState(false);
  const [likeCount, setLikeCount] = useState(0);
  const [likedReply, setLikedReply] = useState(null);
  const router = useRouter();

  useEffect(() => {
    setLiked(comment?.likes?.some((like) => like.userId === userId));
    setLikeCount(comment?.likes?.length);
    const reply = comment?.replies?.map((reply) => ({
      ...reply,
      liked: reply?.likes?.some((like) => like.userId === userId),
    ...
    <div className="flex flex-col text-sm bg-gray-200 min-h-screen">
      ...
      {comment?.reply?.length > 0 &&
        comment?.reply?.map((reply, i) => (
          <div key={i} className="flex flex-col bg-white p-2 w-full shadow-md">
            <div className="flex gap-2 w-full">
              <div className="flex items-center justify-center w-12">
                <Avatar
                  size={40}
            ...
            {/* toolbox */}
            <div className="flex items-center justify-end mt-2">
              <div className="flex items-center gap-1 bg-gray-100 border border-gray-200 px-2 py-1 rounded-s-lg">
                <AiFillHeart className="text-red-500" />
