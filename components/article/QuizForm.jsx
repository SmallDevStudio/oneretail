    ...
    const handleAddSubmit = async () => {
        ...
        try {
            if (selectedQuiz) {
                ...
                await axios.put(`/api/articles/quiz/update?quizId=${selectedQuiz._id}`, newQuiz);
            } else {
    ...
    return (
        <div className="flex flex-col w-full">
            <div className="flex flex-row items-center mt-2 gap-3 w-full">
                ...
                <FaCirclePlus className="flex text-[#0056FF]" size={20}
            ...
            <div className="flex flex-col mt-1">
                {Array.isArray(data) && data.map((quiz, index) => (
                    <div key={index}>
                        <div className="flex flex-row rounded-md px-1 items-center">
                            <span className="flex w-10">{index + 1}</span>
                            <span className="flex w-[250px]">{quiz.question}</span>
