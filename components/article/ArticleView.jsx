...
const ArticleView = ({ articleId }) => {
  ...
  const { data, error, mutate } = useSWR(
    ...
    fetcher,
    {
      onSuccess: (data) => {
        ...
        const initialLikes = {
          [data?.data?._id]: data?.data?.likes?.some(
  ...
  const {
    ...
    `/api/articles/quiz/answers?articleId=${articleId}&userId=${userId}`,
    fetcher
  );

  useEffect(() => {
    if (useAnswer && Array.isArray(useAnswer?.data)) {
      const hasAnswer = useAnswer?.data?.some(
        ...
        <div className="flex flex-col w-full mt-4">
          ...
          <div className="flex flex-col mt-4 w-full">
            ...
            <div className="flex flex-row items-center gap-2 px-4">
              <span className="flex text-sm text-[#0056FF] font-bold">
                Tags:
              </span>
          ...
          <div
            ...
            style={{ textSizeAdjust: "100%", fontSize: "12px" }}
          >
            <div
              className="relative inline-flex items-center columns-2 justify-center p-3 bg-[#F2871F] text-white rounded-full h-6 w-15"
            ...
            <div
              className="relative inline-flex columns-2 justify-center items-center"
                        ...
                        <div className="flex flex-row justify-between items-center">
                          ...
                          {(reply.userId === session?.user?.id ||
                            ...
                            <div className="relative">
                              <BsThreeDotsVertical
                                onClick={(e) =>
                                  handleOptionClick(e, "reply", reply._id)
                                }
                              />
                              <Menu
                                anchorEl={anchorEl}
                      ...
                      <div className="flex flex-row items-center gap-2">
                        {likes[reply._id] ? (
                          <AiFillHeart
                            className="w-3 h-3 text-red-500"
                            onClick={() =>
                              handleReplyLike(reply._id, comment.articleId)
                            }
                          />
                        ) : (
                          <AiOutlineHeart
