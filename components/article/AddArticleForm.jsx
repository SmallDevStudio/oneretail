...
const AddArticleForm = () => {
  ...
  const handleRemoveMedia = async (url, public_id) => {
    try {
      ...
      setMedia((prevMedia) =>
        prevMedia.filter((m) => m.public_id !== public_id)
      );
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!articleId) {
        throw new Error(
          "Article ID is undefined. Article might not have been saved correctly."
        );
      }
      ...
      <div className="flex flex-row w-full items-center justify-between gap-4 mb-2">
        <div className="flex flex-row w-1/3 border-2 rounded-2xl items-center justify-evenly px-4 py-1">
          <div className="flex items-center">
            ...
            <label
              htmlFor="pinned"
              className="ms-2 text-md font-bold text-gray-900"
            >
              ปักหมุด
            </label>
          </div>
      ...
      <div className="flex flex-col w-full gap-4 mt-4">
        ...
        {questions.map((question, index) => (
          <div key={index} className="flex flex-row items-center gap-4">
            ...
            {/* Correctly render the question text */}
            <FaRegEdit
