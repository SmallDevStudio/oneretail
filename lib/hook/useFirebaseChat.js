...
export const useFirebaseChat = () => {
  ...
  const listenToChat = useCallback((chatId) => {
    ...
    const unsubscribe = onValue(chatRef, async (snapshot) => {
        ...
        // โหลดข้อมูลของผู้เข้าร่วมแชท ถ้ายังไม่มี
        if (!participants.length || participants.length !== data.participants.users.length) {
        ...
        const newMessages = await Promise.all(
            Object.entries(messagesData).map(async ([id, message]) => {
                ...
                return {
                    ...
                    likedUsers, // เก็บข้อมูลของผู้กด Like
                };
            })
        );

        // ✅ อัปเดตข้อความแบบสด (ไม่ใช้ prevMessages)
        setMessages(newMessages);

        // โหลดข้อมูลของบทสนทนา
  ...
  const likeMessage = async (chatId, messageId, userId) => {
    try {
        const messageRef = ref(database, `chats/${chatId}/conversation/messages/${messageId}`);
        ...
        if (snapshot.exists()) {
            ...
            const isLike = messageData.isLike || [];

            let updatedLikes;
            if (isLike.includes(userId)) {
                // ✅ ยกเลิกถูกใจ
                updatedLikes = isLike.filter(id => id !== userId);
            } else {
                // ✅ กดถูกใจ
                updatedLikes = [...isLike, userId];
            }

            // ✅ อัปเดตค่า Like ลง Firebase
            await update(messageRef, { isLike: updatedLikes });

            // ✅ Trigger UI Update แบบ Real-Time
            setMessages((prevMessages) =>
                prevMessages.map((msg) =>
                    msg.id === messageId ? { ...msg, isLike: updatedLikes } : msg
                )
            );
        }
    } catch (error) {
