...
export const useFirebaseChat = () => {
  ...
  const listenToChat = useCallback((chatId) => {
    ...
    const chatRef = ref(database, `chats/${chatId}/conversation/messages`);
    
    const unsubscribe = onValue(chatRef, async (snapshot) => {
      if (!snapshot.exists()) {
        setMessages([]); // ถ้าไม่มีข้อความให้เซ็ตเป็นอาร์เรย์ว่าง
        return;
      }
    ...
    return () => unsubscribe(); // ยกเลิก listener เมื่อออกจาก chat
  }, []);
